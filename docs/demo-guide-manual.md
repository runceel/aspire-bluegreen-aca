...
azd env config set infra.parameters.appVersion 1.0.0   # アプリの初期バージョン（手順 5 で 1.1.0 に更新する）
azd env set AZURE_RESOURCE_GROUP rg-prod-manual         # azd は自動設定しないので明示（既定の命名 rg-<env 名> に合わせる）
```

> `appVersion` は web の Docker **ビルド引数**にも使われ、その解決は `infra.parameters.appVersion` を参照します（`subnet`/`sql` などの publish 入力と同じ扱い）。初回 `azd deploy` 前に入れておくことで、ビルド引数の解決失敗（`parameter infra.parameters.appVersion not found`）を防げます。`azd env set`（env 変数）ではビルド引数に反映されないので注意してください。
>
> `AZURE_RESOURCE_GROUP` は azd が ACA アプリを作成するリソースグループです。azd（1.25 系）はこの値を env に書き出さないため、手順 3 以降の `azd env get-value AZURE_RESOURCE_GROUP` が空にならないよう、azd 既定の命名（`rg-<env 名>`）に合わせて先に設定しておきます。
...
foreach ($name in 'web','api') {
  ...
  # 最新リビジョンの抽出。JMESPath の sort_by(@,&...) は使わない: az が az.cmd のため
  # Windows では cmd.exe が "&" をコマンド区切りと解釈して失敗する。PowerShell 側でソートする。
  $latest = (az containerapp revision list -g $appRg -n $app -o json | ConvertFrom-Json |
    Sort-Object { [datetime]$_.properties.createdTime } | Select-Object -Last 1).name
...
バージョン番号は azd パラメータ `appVersion` で渡します（api の `APP_VERSION` env と web のビルド引数の両方に反映され、web/api が同じ version を表示します）。web のビルド引数も `infra.parameters` を参照するため、`azd env config set` で設定します。
