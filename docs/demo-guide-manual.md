...
> `appVersion` はこのアプリの **version の唯一の出所**です。api の `APP_VERSION` env、web の Docker **ビルド引数**、そして各アプリの**リビジョンサフィックス**（bicep で `'v' + replace(appVersion, '.', '-')`、例: `1.0.0`→`v1-0-0`）の導出に使われます。AppHost では既定値なしで公開しているため `{{ parameter "appVersion" }}` として出力され、`infra.parameters.appVersion`（config）から解決されます（`azd env set` の env 変数では解決されない）。既定値が無いので、**初回 `aspire publish` / `az deployment` の前に必ず設定**してください（無いと `parameter infra.parameters.appVersion not found` で失敗）。
>
...
## 3. `aspire publish` と `az deployment` を手動実行する

スクリプト版の `up.ps1` は platform 適用→`aspire publish`→`az deployment`→postdeploy フックを一括実行します。手動実行版では同じ処理を段階的に実行して、内部の動きを確認します。

```powershell
# ① AppHost から Aspire IaC（bicep/bicepparam）を生成
dotnet run --project .\AspireBlueGreen.AppHost\AspireBlueGreen.AppHost.csproj -- publish --output-path .\aspire-publish

# ② 生成物を特定
$bicepFile = (Get-ChildItem .\aspire-publish -Filter *.bicep | Select-Object -First 1).FullName
$bicepParamFile = (Get-ChildItem .\aspire-publish -Filter *.bicepparam | Select-Object -First 1).FullName

# ③ Azure に適用（ACA 環境 + api/web リビジョンを作成）
$appRg = azd env get-value AZURE_RESOURCE_GROUP
az deployment group create -g $appRg -n aspire-initial --template-file $bicepFile --parameters $bicepParamFile
```

> **注意（postdeploy フック）**: 手動の `az deployment group create` は `azure.yaml` の postdeploy フックを実行しません。Front Door 配線とトラフィック検証は、この後の手順 4 で手動実行します。**トラフィック配分（blue=100% / green=0%）はコンテナアプリの bicep が宣言的に行う**ため、`reconcile-traffic.ps1` 相当は検証専用です。
>
> 本書では「中で何が起きているか」を理解するため、**フックを有効にしたまま**、Front Door 配線を `az` コマンドで手動確認し、トラフィックは宣言された状態を `az ... traffic show` で検証します（いずれも安全）。
>
> 完全手動のため、ここでは postdeploy は自動実行されません。下の `az afd ... update` と traffic 確認をそのまま実行してください（トラフィックは bicep が宣言済みなので手動設定は不要です）。
...
Front Door は platform Bicep で profile / endpoint / origin-group（空）/ WAF まで作られています。web の FQDN は `az deployment` 完了後にしか分からないため、**origin の中身だけ**を後から差し込みます。
...
# 初回は create、2 回目以降は update（`configure-frontdoor-origin.ps1` と同じロジック）
$originExists = az afd origin show -g $platRg --profile-name $profile `
  --origin-group-name $og --origin-name web-origin --query name -o tsv 2>$null
$originVerb = if ([string]::IsNullOrWhiteSpace($originExists)) { 'create' } else { 'update' }

az afd origin $originVerb -g $platRg --profile-name $profile --origin-group-name $og --origin-name web-origin `
...
$routeExists = az afd route show -g $platRg --profile-name $profile `
  --endpoint-name $endpoint --route-name web-route --query name -o tsv 2>$null
$routeVerb = if ([string]::IsNullOrWhiteSpace($routeExists)) { 'create' } else { 'update' }

az afd route $routeVerb -g $platRg --profile-name $profile --endpoint-name $endpoint --route-name web-route `
...
リビジョンサフィックスは決定的（`appVersion` から導出）なので、初回デプロイで作られるリビジョンは `web--v1-0-0` / `api--v1-0-0` です。コンテナアプリの bicep が `productionLabel=blue` から **blue ラベル=100%**（green はサフィックス未設定なので省略）を**宣言**しているため、`az deployment` 完了時点で既にトラフィックは正しく設定されています。手動でラベル付けや `traffic set` を行う必要はありません。確認だけします。
...
$bicepFile = (Get-ChildItem .\aspire-publish -Filter *.bicep | Select-Object -First 1).FullName
$bicepParamFile = (Get-ChildItem .\aspire-publish -Filter *.bicepparam | Select-Object -First 1).FullName
az deployment group create -g $appRg -n aspire-v1-1-0 --template-file $bicepFile --parameters $bicepParamFile
```

> `productionLabel` は **blue のまま**変更しません（昇格は手順 7）。
>
> **バージョンは毎回新しくします**。リビジョンサフィックスは `appVersion` から決定的に導出される（`v1-1-0` など）ため、同じバージョンを再 `az deployment` すると `revision with suffix v1-1-0 already exists` で失敗します。デプロイのたびに `appVersion` を上げてください。promote/rollback（手順 7・8）はトラフィックを動かすだけで再デプロイしないため、この制約の影響を受けません。

### 5-1. デプロイ結果の確認（宣言的トラフィック）

`az deployment` は新リビジョン `web--v1-1-0` / `api--v1-1-0` を作成します。コンテナアプリの bicep は `productionLabel=blue` のままなので、**green を 0%、blue を 100%** と宣言します。つまりデプロイの瞬間も本番（blue）からトラフィックは奪われません。手動でラベル付けや `traffic set` を行う必要はありません。確認だけします。
...
web / api を**まとめて** green=100% にします。即時の `az ... traffic set` で切り替えたあと、宣言的状態 `infra.parameters.productionLabel` も green に同期します（これをやらないと、次の `az deployment` が宣言（blue=100%）どおりに**昇格を巻き戻して**しまいます）。
...
# 宣言的状態を同期（次回 az deployment でも green を本番に保つ）
...
- 段階的に流す（カナリア）なら weight を変えるだけ: `--label-weight green=20 blue=80` → 確認後に `green=100 blue=0`。**カナリア中（<100%）は `productionLabel` を変えない／`az deployment` を実行しない**でください（宣言に従って blue=100% に戻ってしまいます）。
...
直前の本番ラベル（blue）へ即時に戻します。**再ビルド不要**、トラフィックを戻すだけです。promote と同様、`infra.parameters.productionLabel` も blue に同期して次回 `az deployment` との整合を保ちます。
...
| `up.ps1` | 上記 + 初期 blue/green パラメータ（`appVersion` / `productionLabel` / `blueRevisionSuffix` / `greenRevisionSuffix`）の `azd env config set` + `aspire publish` + `az deployment` + postdeploy フック |
| `preview.ps1` | `az deployment group what-if` + `aspire publish` + `az deployment --what-if` |
| `bluegreen-deploy.ps1 -Version <x>` | `azd env config set infra.parameters.appVersion <x>` + candidate の `azd env config set infra.parameters.<color>RevisionSuffix v<x>` + `aspire publish` + `az deployment` |
...
| `bluegreen-promote.ps1` | `az containerapp ingress traffic set`（新バージョン側ラベル=100）+ `azd env config set infra.parameters.productionLabel` + `azd env set ACTIVE_LABEL/PREVIOUS_ACTIVE_LABEL` + `aspire publish` + `az deployment` |
| `bluegreen-rollback.ps1` | `az containerapp ingress traffic set`（直前の本番ラベル=100）+ `azd env config set infra.parameters.productionLabel` + `azd env set ACTIVE_LABEL/PREVIOUS_ACTIVE_LABEL` + `aspire publish` + `az deployment` |
